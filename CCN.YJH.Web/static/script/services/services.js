var services = angular.module('R5E.services', ['ngResource']);
/**
* 提交后台获取数据的服务
**/
...
services.factory('SendMobileCodeService', ['httpService', '$q', function (httpService, $q) {
    return {
        send: function (mobile, storid) {
            var deferred = $q.defer();
            httpService.setting('../../server/R5ELottery.ashx', { "mobile": mobile, "action": "3", "storid": storid }).then(function (data, status, headers, config) {
                deferred.resolve(data, status, headers, config);
            });
            return deferred.promise;
        },
        check: function (mobile, verify) {
            var deferred = $q.defer();
            httpService.setting('../../server/VerififyMobileCode.ashx', { "mobile": mobile, "mobilecode": verify }).then(function (data, status, headers, config) {
                deferred.resolve(data, status, headers, config);
            });
            return deferred.promise;
        }
...
///
services.factory('UserSessionExpire', ['httpService', '$q', function (httpService, $q) {
    return {
        query: function () {
            var deferred = $q.defer();
            httpService.setting('../../server/userSessionExpire.ashx', { "type": "userSession" }).then(function (data, status, headers, config) {
                deferred.resolve(data, status, headers, config);
            });
            return deferred.promise;
        }
